  ...
  return (
    <article className="rounded-2xl border border-[#d9dde4] bg-[#f6f7f9] p-5 shadow-[0_8px_18px_rgba(10,18,35,0.06)]">
      ...
      <div className="mt-4 rounded-xl border border-[#d9dde4] bg-[#f2f4f7] px-4 py-3">
        <p className="text-base font-semibold text-[#0f2431]">Evaluasi Teknis Kuantitatif</p>
        ...
        <table className="min-w-full border-separate border-spacing-0 text-sm">
          <thead>
            <tr className="text-left text-[#60717e]">
              ...
              <th className="border-b border-[#d9dde4] px-3 py-2 font-medium">Skor Maks</th>
              <th className="border-b border-[#d9dde4] px-3 py-2 font-medium">% Bobot</th>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.id}>
                ...
                <td className="border-b border-[#e5e9ef] px-3 py-3">{row.maxScore}</td>
                <td className="border-b border-[#e5e9ef] px-3 py-3">{row.weight}</td>
                {vendorColumns.map((vendor) => (
                  <td key={`${row.id}-${vendor.id}`} className="border-b border-[#e5e9ef] px-2 py-2">
                    <input
                      readOnly
                      value="0"
                      className="w-full rounded-lg border border-[#d9dde4] bg-[#f2f4f7] px-2 py-1.5 text-center text-sm text-[#60717e]"
                    />
                  </td>
                ))}
              </tr>
            ...
            <tr>
              <td colSpan={4} className="px-3 py-3 text-right font-semibold text-[#0f2431]">
                Total Skor:
              </td>
              {vendorColumns.map((vendor) => (
                <td key={`total-${vendor.id}`} className="px-2 py-3 text-center">
                  <span className="inline-flex rounded-full bg-[#dbe7ff] px-3 py-0.5 text-xs font-semibold text-[#153c7a]">
                    {vendor.score}
                  </span>
                </td>
              ))}
